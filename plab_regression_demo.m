...
%% Overlapping additive responses to different events
...
%% Decoding regression, and ridge regression

% The above regression examples show the "encoding" direction: we create
% responses to events, and try to sum those responses to try to predict our
% activity. 

% We can also use regression in the "decoding" direction: use activity to
% predict when an event happens. 

% We first used this in Song, Peters bioRxiv 2025 - check out Extended Data
% Figure 2 for a schematic of how this works. 

% Encoding and decoding regression have different goals: encoding seeks to
% explain activity with events, decoding seeks to predict events from
% activity. Encoding is therefore "clean" only if you give enough
% regressors to accurately predict activity, while decoding can be "clean"
% as long as the activiy pattern to the decoded event is consistent and
% unique.

% For example: let's say we want to find the widefield response to the
% stimulus in our wheel task. After learning, mice turn the wheel shortly
% after the stimulus onset, so stimulus and movement activity is jumbled
% together.

% If we use encoding regression including regressors for stimulus and
% movement onset, single kernels will be made for stimulus and movement.
% This one-size-fits-all doesn't work for movement though: movement
% activity can be very variable e.g. for different speeds of movement. This
% means that in the encoding regression, some movement activity will
% "bleed" into the stimulus kernel. 

% Instead, we can use decoding regression to get a cleaner kernel. This
% only works because movement is *not* uniquely done after stimuli in our
% task: fortunately, mice also turn the wheel a lot during the ITI. This
% means that movement-related activity is not predictive of a stimulus
% onset: only stimulus-related activity is. Our stimulus decoding kernel
% will therefore ignore all movement-related activity, even if it's a
% little different on every trial. The decoding method therefore needs less
% information (/makes fewer assumptions) than the encoding method in this
% case. 

% We can do decoding regression by just swapping the order of regressors
% and kernels: instead of using events as the regressors to predict
% activity as our signal, we now want activity as our regressors to predict
% events as our signal. 

% A new parameter is needed for this: lambda, the regularization parameter.
% When we flip activity and events in our regression, we now have a lot of
% regressors (e.g. 200 V components). This means the regression will
% overfit the data because it has so many options to choose from. We can
% fix this by adding a term to penalize higher weights. This forces weights
% to be more evenly disributed across the regressors, which counters
% overfitting that results in having some weights be massive and some be
% near zero. The value for lambda is arbitrary, and relates to the scale of
% the signal that's being predicted. 

% Another new feature helps this regression: adding a constant as a
% regressor. This just fits one value to offset all prediction results,
% essentially as if it's setting a "baseline" for activity to predict the
% event. 

% Here's the syntax of how to incorporate these into ap.regresskernel: 
lambda = 10; % lambda regularization value
use_constant = true; % flag to include a constant in the regression
kernels = ap.regresskernel(activity,events,time_shifts,lambda,[],[],[],use_constant);

% [EXERCISE]
% Use decoding regression to generate a stimulus kernel for the loaded
% dataset. Try a range of lambda values. 
% - How does the decoding stimulus kernel compare to the encoding stimulus kernel?
% - How does changing the lambda value affect the kernel?


%% Regressing widefield to multiunit spikes

% The above examples show regression between activity (continuous data) and
% events (discrete binary data). We can also regress continuous data to
% other continuous data, e.g. widefield fluorescence to multiunit spikes
% (as in Peters et al. Nature 2021).

% [EXERCISE]
% 1) Load this data, which includes widefield imaging and striatal ephys:
animal = 'AP023';
rec_day = '2024-09-09';
rec_time = '1435';
verbose = true;

% 2) Create a multiunit signal by binning all spikes between 1500-2000 um
% depth. Use time bins to match the widefield frames, so that you have one
% timepoint for ephys for each timepoint for widefield.

% 3) Regress widefield to ephys (e.g. predict multiunit spiking using
% calcium fluorescence). Include time lags of -200ms:+200ms, and try using
% a range of lambda values. 

% 4) What lambda value is the best? For continuous data, we can answer this
% by cross validation: train our kernel on some data, test our fits on the
% rest of the data, quantify how much variation was explained. The higher
% the explained variance, the better our model. 
%
% The syntax to do that in ap.regresskernel is this: 
cvfold = 5; % split the data into this many train/test segments
[kernels,~,explained_variance] = ...
    ap.regresskernel(activity,events,time_shifts,lambda,cvfold,[],[],use_constant);

% The explained_variance has two fields: .total for all explained variance,
% .partial for each regressor. All we need to care about here is
% explained_variance.total.
%
% Try running regression with a range of lambda values, and save the
% explaned variance corresponding to each lambda. Plot the explained
% variance vs. lambda. Does this tell you what the "best" lambda value is?

% 5) Make multiunit signals as above for multiple depths: bin spikes from
% 1500um in 500um intervals until the end of the probe (to have 5 multiunit
% signals by depth). Run regression on all of these (this can be done
% simultaneously, just use a matrix of depth x time as the signal to be
% predicted). 
% - What do you notice about the kernels? 
% - What's the biological meaning of these kernels?
